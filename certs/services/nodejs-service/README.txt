Certificate Package: nodejs-service
Generated: 2026-06-03 14:46 UTC
Valid for: 365 days
CA: TranscendenceCA

FILES
─────
nodejs-service.key          RSA private key      — DO NOT DISTRIBUTE (chmod 600)
nodejs-service.crt          Signed certificate   — safe to distribute
nodejs-service.pem          key + cert bundle    — nginx, HAProxy, raw TLS (chmod 600)
nodejs-service-chain.pem    cert + CA bundle     — give to clients for mutual TLS trust
nodejs-service.p12          PKCS12 keystore      — Spring Boot server.ssl.*
...
server:
  ssl:
    enabled: true
    key-store:             classpath:nodejs-service.p12
    key-store-password:    ${CERT_PASSWORD:password}
    key-store-type:        PKCS12
    key-alias:             nodejs-service
