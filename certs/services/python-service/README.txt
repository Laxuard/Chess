Certificate Package: python-service
Generated: 2026-06-03 14:53 UTC
Valid for: 365 days
CA: TranscendenceCA

FILES
─────
python-service.key          RSA private key      — DO NOT DISTRIBUTE (chmod 600)
python-service.crt          Signed certificate   — safe to distribute
python-service.pem          key + cert bundle    — nginx, HAProxy, raw TLS (chmod 600)
python-service-chain.pem    cert + CA bundle     — give to clients for mutual TLS trust
python-service.p12          PKCS12 keystore      — Spring Boot server.ssl.*
...
server:
  ssl:
    enabled: true
    key-store:             classpath:python-service.p12
    key-store-password:    ${CERT_PASSWORD:password}
    key-store-type:        PKCS12
    key-alias:             python-service
